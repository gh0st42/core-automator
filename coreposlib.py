...
def get_session():
    ...
    (ret, val) = subprocess.getstatusoutput(
        "coresendmsg --tcp -l SESSION flags=STRING,TEXT")
...
def get_nodes_by_id(session_number):
    nodes = {}
    lines = [line.rstrip('\n') for line in open(
        "/tmp/pycore." + session_number + "/nodes")]
    for l in lines:
        entries = l.split(" ")
...
def play_step(scenario, step_number):
    ...
    for node in step_list[step_number]:
        if node[1] != '-' and node[2] != '-':
            cmd = "coresendmsg NODE NUMBER=" + \
                node[0] + " X_POSITION=" + node[1] + " Y_POSITION=" + node[2]
            (ret, val) = subprocess.getstatusoutput(cmd)
        if node[4] != "":
            cmd = "vcmd -c /tmp/pycore." + session + "/" + \
                node[3] + " -- bash -c '" + node[4] + "'"
            (ret, val) = subprocess.getstatusoutput(cmd)
    ...
    with open(filename, mode) as out:
        for f in glob.glob("/tmp/pycore." + session + "/*.xy"):
            ...
            with open(f, "r") as xy:
                ...
                out.write(
                    node_name + " " + coord[0].split(".")[0] + " " + coord[1].split(".")[0] + "\n")
        out.write("-- STEP\n")
